document.addEventListener("DOMContentLoaded", function () {
  ...
  // Thêm hàm này vào trong file archive.js (bên trong hoặc ngoài DOMContentLoaded đều được vì nó gán vào window)

  window.clearAllArchive = function () {
    // Hiển thị cảnh báo xác nhận để tránh xóa nhầm
    const confirmation = confirm(
      "BẠN CÓ CHẮC CHẮN? Thao tác này sẽ xóa toàn bộ danh sách ghi chú đã lưu và không thể hoàn tác."
    );

    if (confirmation) {
      // Xóa key lưu trữ ghi chú trong localStorage
      localStorage.removeItem("favNotes");

      // Nếu bạn định nghĩa renderArchive bên trong DOMContentLoaded,
      // hãy đảm bảo hàm này có thể truy cập được hoặc dùng location.reload()
      location.reload();
    }
  };
